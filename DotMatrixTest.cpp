...
const uint8_t PROGMEM PATTERN_DFROBOT[] =
{ 0x00, 0xff, 0xff, 0x81, 0x81, 0x81, 0x7e, 0x00, // D
		0x00, 0xff, 0xff, 0x90, 0x90, 0x90, 0x90, 0x00, // F
		0x00, 0xff, 0xff, 0x90, 0x9c, 0x97, 0xf1, 0x00, // R
		0x00, 0xff, 0xff, 0x81, 0x81, 0xff, 0xff, 0x00, // O
		0x00, 0xff, 0xff, 0x91, 0x91, 0xf1, 0x1f, 0x00, // B
		0x00, 0xff, 0xff, 0x81, 0x81, 0xff, 0xff, 0x00, // O
...
void animationRotateDFRobot(word k)
{
	k %= 28 * 2 * 7;
	...
	if (k % 56 == 0)
	{
		...
		{
			byte value = DotMatrix::reverseByte(
					pgm_read_byte_near(PATTERN_DFROBOT+(k/56)*8+i));
			dm.setByte(0x18 + i, value);
			dm.setByte(0x20 + i, value);
		}
	}
...
void loop()
{
	...
	// Amination for rotate "DFROBOT" logo, other animation could be commented out.
	callAnimation(animationRotateDFRobot, 0x04, 28 * 2 * 7, 0x00,
			Driver_3D8::ZXY);
